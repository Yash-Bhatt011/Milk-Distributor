...
exports.getReports = async (req, res) => {
    try {
        const distributorStats = await User.aggregate([
                    ...
                    activeCustomers: {
                        $size: {
                            $filter: {
                                input: '$customers',
                                as: 'customer',
                                cond: { $eq: ['$$customer.customerFields.active', true] }
                            }
                        }
                    },
                        ...
                        $multiply: [
                            {
                                $divide: [
                                    ...
                                    { $cond: [{ $eq: [{ $size: '$deliveries' }, 0] }, 1, { $size: '$deliveries' }] }
                                ]
                            },
                            100
                        ]
        ...
        const summary = {
            totalCollections: distributorStats.reduce((sum, d) => sum + (d.totalRevenue || 0), 0),
            totalOutstanding: distributorStats.reduce((sum, d) => sum + (d.outstandingAmount || 0), 0),
            averageCollection: 0,
            collectionRate: 0
        };
